...
(function(){
  ...
  .run(["$templateCache", function($templateCache) {
    $templateCache.put("carousel-directive.html","<div class='jk-carousel' > <div class='slides-container' layout='row' >   <div      ng-repeat='slideItem in ctrl.cloneData'      class='slide'    >      <div ng-include='ctrl.itemTemplateUrl' ></div>    </div>  </div>  <md-button class='md-icon-button left-arrow-button' >    <md-icon ng-click='ctrl.navigateLeft()' style='margin:-10px;padding:-10px'>chevron_left</md-icon>  </md-button>  <md-button class='md-icon-button right-arrow-button' >    <md-icon style='margin:-15px;padding:-10' ng-click='ctrl.navigateRight()' >chevron_right</md-icon>  </md-button>  <md-radio-group    class='radio-buttons-container'    layout='row'    ng-model='ctrl.radioButtonIndex'    layout-align='center center'    ng-change='ctrl.onRadioButtonClick()' >    <md-radio-button class='md-primary'      ng-repeat='item in ctrl.data'      ng-value='$index'      aria-label='$index' >    </md-radio-button>  </md-radio-group></div>");}]);})();
